...
/**
 * Runs work() in only one browser tab at a time. Web Locks release automatically
 * when a tab closes, so the lock migrates to another open tab on its own.
 * ifAvailable:true means a tab that doesn't win the lock skips this cycle rather
 * than queuing behind the winner and running late.
 * @param {String} lock_name the shared name contending tabs compete for.
 * @param {Function} work async function to run if this tab holds the lock.
 * @param {String} label log prefix used when the lock is unavailable, e.g. '[RQ Monitor]'.
 */
async function runIfLeader(lock_name, work, label = '[RQ]') {
  ...
  await navigator.locks.request(lock_name, { ifAvailable: true }, async lock => {
    if (!lock) { console.log(`${label} Another tab is polling; skipping this cycle`); return; }
    await work();
  });
}

/**
 * Builds a retention pruner for background pollers that keep one localStorage key
 * per record. Without pruning, every record ever polled leaves a key behind and the
 * origin's ~5MB quota is eventually exhausted, which breaks writes across all of
 * RentalQuirks — and a poller that only warns on a failed write then degrades into
 * silence rather than failing visibly.
 *
 * Retention is keyed on when a record was last seen, tracked in a registry at
 * seen_key ({ id: lastSeenMs }), rather than on absence from the current batch:
 * the prep-sheet reads swallow per-date errors and return [], so a partially failed
 * read must not be allowed to discard state for records that are still live.
 *
 * @param {String} seen_key localStorage key holding the last-seen registry.
 * @param {String[]} prefixes per-record key prefixes this pruner may delete.
 * @param {Number} retention_ms how long a record's state outlives its last sighting.
 * @param {String} label log prefix, e.g. '[RQ Monitor]'.
 * @returns {Function} prune(ids) — marks ids as seen now, then deletes expired keys.
 */
function makeStatePruner(seen_key, prefixes, retention_ms, label) {
  return function prune(ids) {
    const now = Date.now();
    let seen;
    try { seen = JSON.parse(localStorage.getItem(seen_key) || '{}'); }
    catch { seen = {}; }

    ids.forEach(id => { seen[id] = now; });

    const live = new Set();
    for (const [id, last_seen] of Object.entries(seen)) {
      if (now - last_seen < retention_ms) live.add(id);
      else delete seen[id]; // keeps the registry itself bounded
    }
    ...
    catch (e) { console.warn(`${label} Failed to save seen-record registry`, e); }

    if (dropped) console.log(`${label} Pruned ${dropped} stale key(s)`);
  };
}